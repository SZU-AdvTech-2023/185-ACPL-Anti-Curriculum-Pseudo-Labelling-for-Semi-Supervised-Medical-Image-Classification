...
class PLUL:
    def __init__(
        ...
        self.local_info = self.build_local_graph(  #从有标签数据集中找到距离无标签数据集中每一个数据最近的50个数据的距离和其索引
            x_info["embed1"].cpu().numpy(),
            u_info["embed1"].cpu().numpy(),
            args,
        )

        self.local_ds = self.get_ds()  #距离均值平均归一化
        self.idxs_pack = self.build_GMM(num_gmm_sets=num_gmm_sets, fig=True)

        self.sel = self.idxs_pack[args.sel] #最有信息量的数据的索引
        self.local_agg = self.get_agg()

        self.local_info_r = self.build_local_graph(     #和上面反过来
            u_info["embed1"].cpu().numpy(), x_info["embed1"].cpu().numpy(), args
        )

    def anchor_purify(self):
        result_idxs = []
        for i in range(self.local_info["i"][self.sel].shape[0]):     #无标签数据中的高信息的数据
    ...
    def build_local_graph(self, x_embed, u_embed, args): #搜索最近邻使用KNN ，相当于是无标签数据在有标签数据集里搜索到最近的50个数据，保存
        # GPU                                                                                    
        ...
        D_ulf, I_ulf = gpu_index_ulf.search(u_embed, args.topk) #topk默认是50
        # D_ulf, I_ulf = index_ulf.search(u_embed, args.topk)
        # del gpu_index_ulf
        return edict({"d": D_ulf, "i": I_ulf})  #D_ulf存的是距离，I_ulf存的是索引
    ...
    def get_ds(self):  #获取到每个样本到其他样本距离的平均值，然后归一化
        ds = self.local_info["d"].mean(1)
        ds = (ds - ds.min()) / (ds.max() - ds.min())
        ds = ds.reshape(-1, 1)
        return ds

    def get_agg(self):  #每个无标签数据的最近的数据的标签的均值
        knn_gts = self.x_info["gts"][self.local_info["i"]].cpu().numpy() #获得每个无标签样本距离最近的50个有标签样本的标签
        agg = knn_gts.mean(1)       
        return edict({"knn_gts": knn_gts, "agg": agg})

    def get_pseudo(self, target, val=True, num_class=14):  #改了这里val = False!!!!!!!!
        ...
        if self.ds_mixup:
            ...
            weight = 1.0
        mix = weight * pred + (1 - weight) * agg  #模型预测的均值和最近的均值
    ...
    def build_GMM(self, num_gmm_sets=3, fig=False, name="Local Density"):
        ...
        gmm1.fit(target)
        pred = gmm1.predict(target)   #本文是把数据分成了三类，所以用三个混合高斯，来进行聚类预测
        ...
        if fig and dist.get_rank() == 0:
            ...
            if num_gmm_sets == 3:
                plt.hist(
                    ...
                    edgecolor="black",
                    alpha=0.5,
                    label=f"Medium Informativeness",
                )
            plt.legend()
            plt.grid()
