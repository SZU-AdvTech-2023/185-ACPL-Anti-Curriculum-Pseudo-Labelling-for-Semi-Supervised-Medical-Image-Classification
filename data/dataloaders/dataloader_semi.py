...
class ChestDataset(Dataset):
    def __init__(self, root_dir, transform, mode, runtime=1, ratio=100) -> None:
        ...
        self.labeled_gr = np.zeros((labeled_gr.shape[0], 15))    #将图像的标签转化为二进制编码
        ...
        unlabeled_gr = np.asarray([gr[i] for i in self.unlabeled_imgs])#将无标签图像的标签转化为二进制编码
    ...
    def x_add_pl(self, pl, idxs):          #加入无标签数据到有标签数据集
    ...
    def u_update_pl(self, idxs):  #删除操作，从无标签数据集删除
