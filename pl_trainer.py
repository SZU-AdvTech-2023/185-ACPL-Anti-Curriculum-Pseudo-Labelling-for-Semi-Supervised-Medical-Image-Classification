...
class ACPL_Trainer:
    def __init__(self, args):
        ...
        self.best_loop = 0
        self._transform_init(args) #初始化转化方法
        self._loader_init(args)  #初始化loader和sampler

        self._log_init(args) #初始化日志

        self._model_init(args)#初始化模型
        self._optim_init(args) #初始化Adam优化器

        self._crit_init(args, self.label_loader1) #初始化损失函数，二元交叉损失函数
    ...
    def pipeline_master(self, args):
        if args.resume is None:
            ...
            self.best_loop = 0
        self._ck_load(args, self.best_iter, self.best_loop) #加载模型参数和优化器状态的函数
        # self.test(args, self.best_iter)

        anchor = self._anchor_ext(args)   #锚点集运行经验推断
        if dist.get_rank() == 0:
        ...
        for i in range(1, args.total_loops + 1):
            ...
            unlabel = self._anchor_sim(args)   #在无标签数据集上进行模型推断，将结果保存在各个张量中
            if dist.get_rank() == 0:
    ...
    def _anchor_sim(self, args):  #得到无标签数据集的各种信息，比如真实的标签和索引
        self.net1_ema.eval()
        ...
        with torch.no_grad():        #inputs是输入的数据
    ...
    def _anchor_ext(self, args):   
        ...
        with torch.no_grad():        #遍历加载器获得每个批次的输入数据，以及数据对应的标签，索引
    ...
    def _log_init(self, args):     #保存日志和模型检查点目录，嵌入向量目录
        log_base = f"{args.log_base}/{args.task[0]}/{args.desc}"     #构建日志目录
        print(log_base)
    ...
    def _transform_init(self, args):      #数据增强，数据裁剪和随机旋转，归一化
    ...
    def _loader_init(self, args):     #获得数据集以及loader和sampler
        if self.task == "cx14":
    ...
    def _create_model(self, arch, num_class, ema=False, imagenet=True):  #
        ...
        in_features = 1024
        model1.classifier = nn.Linear(in_features, num_class) #具体地，将模型的 classifier 层替换为一个新的全连接层，输出维度是 num_class，以适应任务的类别数
        # model1.fc = nn.Linear(in_features, num_class)

        model1 = nn.SyncBatchNorm.convert_sync_batchnorm(model1)
        return model1

    def _model_init(self, args):       #初始化主干模型
        backbone = densenet.densenet121         #net1是主干模型，net1_ema是指数滑动平均模型
    ...
    def _optim_init(self, args):    #初始化优化器
        ...
        self.optimizer1_ema = WeightEMA(self.net1, self.net1_ema)    #加权指数移动平均的优化器，用于在训练中更新模型参数
